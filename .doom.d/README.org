...
(use-package! tab-bar
  ...
  :custom-face
  (tab-bar-tab ((t (:inherit ,(if (eq (car custom-enabled-themes) 'vim-colors)
                                  nil
                                doom-modeline-panel)
                    :foreground nil
                    :background ,(if (eq (car custom-enabled-themes) 'vim-colors)
                                     "dark gray"
                                   nil)))))
  (tab-bar-tab-inactive ((t (:foreground nil
                             :background nil))))
...
(defun bk/tab-bar-setup ()
  ;; Tab 1
  (progn (tab-bar-rename-tab "EmacsConf")
...
(use-package! company
  ...
  (map!
   ...
   [(control) (meta) ?s] #'company-search-candidates))
#+end_src
...
Some customizations to the =vim-colors= theme.
#+begin_src emacs-lisp
(defun bk/vim-colors-theme-customizations ()
  (interactive)
  (custom-set-faces
   '(org-block-begin-line ((t (:underline "#A7A6AA"
                               :foreground "#008ED1"
                               :background "#EAEAFF"))))
   '(org-block ((t (:background "white smoke"))))
   '(org-block-end-line ((t (:overline "#A7A6AA"
                             :foreground "#008ED1"
                             :background "#EAEAFF"))))))

(bk/vim-colors-theme-customizations)
#+end_src
...
Turn off all the org's eye candy that Doom emacs has
#+begin_src emacs-lisp
(after! org
...
This is my attempt to provide such feature using =run-with-timer= so the next
    ...
    (let ((alert-msg
           (if (= (length new-notifs) 1)
               `((msg . ,(oref (-first-item new-notifs) title))
                 (title . ,(format "New %s in repo: %s"
                                   (oref (-first-item new-notifs) type)
...
(defun alert-gh-notify--callback ()
  ...
  (setq gh-notify--forge-last-timestamp (format-time-string "%Y-%m-%dT%H:%M:%SZ"))
  ...
  (if alert-gh-notify--current-timer
    ...
    (progn
      (setq alert-gh-notify--current-timer
            (run-with-timer alert-gh-notify-idle-period
...
#+begin_src emacs-lisp
;; (add-hook 'after-init-hook #'alert-gh-notify-start)
#+end_src
...
(defun bk/delete-worktree ()
  ...
  (interactive)
  (let ((worktree (completing-read "Choose worktree: " (magit-list-worktrees))))
    (mapc (lambda (buffer)
