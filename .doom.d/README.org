...
something completely unpredictable from the creators of the original software.
...
Do not automatically tangle the =config.org= file because it is too slow for me
and introduce a lot of undesired noise.
#+begin_src emacs-lisp
  ...
  (save-excursion
    (if arg
        (progn
          ...
          (let ((res (y-or-n-p "Add word at point? ")))
            (if res
                (+spell/add-word)
              (+spell/correct))))
        (progn
