...
- [[#apps][Apps]]
  ...
  - [[#irc][IRC]]
  - [[#ledger][Ledger]]
  - [[#easy-templates-1][Easy Templates]]
...
(add-to-list
 'org-capture-templates
 `("lc" "CreditCard" plain (file ,(format "~/org/ledger/ledger-%s.dat" 2022 ;; (format-time-string "%Y")
                                    )),
...
(setq
 ...
 '(("netcash" "ledger [[ledger-mode-flags]] -f %(ledger-file) -R -X R$ --current bal ^assets:bank ^assets:crypto liabilities:creditcard")
   ...
   ("budget" "ledger [[ledger-mode-flags]] -f %(ledger-file) -X R$ --current bal ^assets:bank:checking:budget liabilities:card")
