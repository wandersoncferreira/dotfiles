...
(defun alert-gh-notify--new-notifications ()
  (emacsql-with-transaction (forge-db)
    (let* ((raw-notifs (forge-sql
      ...
      (-filter
       (lambda (n)
         (not (-contains-p alert-gh-notify--last-ids (oref n id))))
       forge-notifs))))
...
I'm using =gh-notify= just fine! However, I need to periodically call the refresh
function and to use the =alert= package to notify me about new msgs.
